...
- **Repo ingestion + Fly.io migration** (v0.17.0, 2026-05-17) — `aitrace audit-repo <url>` and POST /audit/repo accept a public GitHub URL, discover trace artifacts and governance docs via a 12-entry manifest, and produce a combined compliance report. Hosting moved from Railway to Fly.io.
...
Tested `aitrace scan` on 5 open-source projects (Haystack, CrewAI, LiteLLM, n8n, Dify) and submitted PRs. Automated code review (Greptile) and human reviewers flagged three product-level bugs:
...
**Fix:** Add an Annex III risk classification section to `aitrace docs` and `aitrace scan` output. List the high-risk categories. Direct non-high-risk users to Article 50 and GDPR sections only.
